...
site = ["昌平"]
shop = ["麻辣烫店"]
dz = random.choice(site)
sp = random.choice(shop)
while True:
    d = int(input("输入’任意整数‘进行选择您去的地址："))
    print(dz)
    if dz in dict_shop:
        d2 = int(input("输入’任意整数‘选择您的商铺："))
        print(sp)
        if sp in dict_shop[dz]:
            print("恭喜您拿到饭了，开始送货")
            if dict_shop[dz]["客户地址"] == dict_client["昌平"]["翻斗花园"]:
                dict_client["昌平"]["外卖"] = "麻辣烫"
                money = money+dict_shop[dz]["跑腿费"]
